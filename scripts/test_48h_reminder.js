  ...
  sale: {
    async findMany({ where }) {
      return Array.from(sales.values())
        .map((sale) => {
          if (returnStaleFetch)
            return { ...sale, status: "processed", vesperaSentAt: null };
          return sale;
        })
        .filter((sale) => {
          const outbound = new realDate(sale.outboundDate).getTime();
          return (
            sale.status === where.status &&
...
notificationService.sendPurchaseReminderNotification = async (
  customer,
  sale,
  link,
) => {
...
async function runAt(hoursFromBase) {
  currentTime =
    realDate.parse("2026-08-23T12:00:00.000Z") + hoursFromBase * 60 * 60 * 1000;
  reminders.length = 0;
  updates.length = 0;
  sale.vesperaSentAt = null;
...
(async () => {
  assert.deepStrictEqual(
    await runAt(-2),
    { reminders: 0, updates: 0 },
    "50h before must not send",
  );
  assert.deepStrictEqual(
    await runAt(-1),
    { reminders: 0, updates: 0 },
    "49h before must not send",
  );
  assert.deepStrictEqual(
    await runAt(0),
    { reminders: 1, updates: 1 },
    "48h before must send",
  );
  assert.deepStrictEqual(
    await runAt(1),
    { reminders: 1, updates: 1 },
    "47h before must send",
  );
  assert.deepStrictEqual(
    await runAt(2),
    { reminders: 1, updates: 1 },
    "46h before must catch up an unsent notification",
  );
  ...
  assert.strictEqual(
    reminders.length,
    1,
    "a sent reminder must be deduplicated",
  );
  ...
  await runOutboundNotifications();
  assert.deepStrictEqual(
    { reminders: reminders.length, updates: updates.length },
    { reminders: 0, updates: 0 },
    "cancelled sales must not receive reminders",
  );
  ...
  assert.deepStrictEqual(
    { reminders: reminders.length, updates: updates.length },
    { reminders: 0, updates: 0 },
    "sandbox partners must be suppressed",
  );
  ...
  await runOutboundNotifications();
  assert.strictEqual(
    physicalTagReceipts.length,
    1,
    "physical tag receipt must be sent once in the 48h window",
  );
  assert(
    physicalTagOrders[0].receiptSentAt,
    "physical tag receipt must be marked as sent",
  );

  const template = fs.readFileSync(
    path.resolve(__dirname, "..", "email_template_confirmacao_compra.html"),
    "utf8",
  );
  assert(template.includes("Compra e reserva confirmadas"));
  assert(
    template.includes("{{texto_entrega}}"),
    "confirmation template must render the reservation variant",
  );
  assert(template.includes("Orientações gerais"));
  assert(
    !template.includes("Ã"),
    "confirmation template must not contain mojibake",
  );
