...
const saleServicePath = require.resolve('../src/services/saleProcessingService');
const callbackServicePath = require.resolve('../src/services/partnerCallbackService');
...
const fakeAgenda = {
  create(name, data) {
    return {
      attrs: { name, data },
      unique(value, options) { this.uniqueValue = value; this.uniqueOptions = options; return this; },
      schedule(value) { this.attrs.nextRunAt = value; return this; },
      async save() { this.attrs._id = '11111111-1111-4111-8111-111111111111'; return this; }
    };
  },
  async queryJobs() { return queryResult; },
  databasePool: { async query() { return { rowCount: 0 }; } }
};
require.cache[agendaClientPath] = { id: agendaClientPath, filename: agendaClientPath, loaded: true, exports: fakeAgenda };
...
require.cache[saleServicePath] = {
  ...
  loaded: true,
  exports: { processPartnerSale: async data => { processedSale = data; } }
};
...
require.cache[callbackServicePath] = {
  ...
  loaded: true,
  exports: {
    executeCallback: async data => { callbackInput = data; return callbackResult; }
  }
};
...
(async () => {
  const saved = await jobService.enqueueUniqueJob({
    ...
    data: { partnerId: 'PARTNER', saleId: 'SALE-1', customerEmail: 'secret@example.com' },
    dedupeKey: 'processPartnerSale:PARTNER:SALE-1',
    maxAttempts: 3
  });
  assert.strictEqual(String(saved.attrs._id), '11111111-1111-4111-8111-111111111111');
  assert.deepStrictEqual(saved.uniqueValue, { 'data.dedupeKey': 'processPartnerSale:PARTNER:SALE-1' });
  ...
  queryResult = {
    jobs: [{
      ...
      name: 'processPartnerSale',
      data: { partnerId: 'PARTNER', dedupeKey: 'processPartnerSale:PARTNER:SALE-1' }
    }],
    total: 1
  };
  ...
  assert.strictEqual(String(duplicate.attrs._id), '22222222-2222-4222-8222-222222222222');
  ...
  await assert.rejects(
    () => partnerCallbackHandler({ attrs: { data: { partnerId: 'PARTNER', maxAttempts: 2 }, failCount: 0 } }),
    /retry requested/
  );
  callbackResult = { success: false, retry: false };
  await assert.rejects(
    () => partnerCallbackHandler({ attrs: { data: { partnerId: 'PARTNER', maxAttempts: 2 }, failCount: 1 } }),
    /attempts exhausted/
  );
  ...
  const serialized = jobService.serializeJob({
    ...
    data: {
      ...
      payload: { event: 'customer.registration.completed', saleId: 'SALE-1', signature: 'secret' },
  ...
  assert.strictEqual(jobService.mapAgendaStatus({ state: 'running', lockedAt: new Date(), failCount: 1, nextRunAt: new Date() }), 'running');
  ...
  assert.strictEqual(
    await jobService.getJobById('11111111-1111-4111-8111-111111111111', 'PARTNER'),
    null,
    'partners must not inspect each other jobs'
  );
  assert.strictEqual(await jobService.getJobById('legacy-job-id', 'PARTNER'), null);
