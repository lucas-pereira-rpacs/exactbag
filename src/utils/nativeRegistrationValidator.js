...
const toBoolean = (value) =>
  value === true ||
  value === 1 ||
  ["true", "1", "yes", "sim"].includes(String(value).toLowerCase());

const validateCpf = (cpf) => {
  if (!cpf) return { valid: false, error: "CPF é obrigatório" };
  const clean = cpf.replace(/\D/g, "");
  if (!CPF_REGEX.test(clean))
    return { valid: false, error: "CPF inválido (11 dígitos)" };
...
const validateRegistrationInput = (body) => {
  ...
  if (
    ...
    body.passengerEmail !== body.emailConfirmation
  ) {
    errors.push({
  ...
  if (!phone || !PHONE_REGEX.test(phone)) {
    errors.push({
      ...
      message: "Telefone inválido (10-15 dígitos)",
    });
  }
  ...
  if (!body.destination || String(body.destination).trim().length < 2) {
    errors.push({
      field: "destination",
      message: "Destino da viagem é obrigatório",
    });
  }
  ...
  if (toBoolean(body.isPhysicalTag || body.physicalTag) && !body.sunNumber) {
    errors.push({
      field: "sunNumber",
      message: "O número SUN da TAG é obrigatório",
    });
  }
  ...
  if (isNaN(baggageQty) || baggageQty < 1 || baggageQty > 10) {
    errors.push({
      field: "baggageQty",
      message: "Quantidade de bagagens deve ser entre 1 e 10",
    });
  }
    ...
    body.baggageItems.forEach((item, idx) => {
      if (item.imageData && typeof item.imageData === "string") {
        ...
        if (item.imageData.length > 7_000_000) {
          errors.push({
            field: `baggageItems[${idx}].imageData`,
      ...
      if (item.imageData2 && typeof item.imageData2 === "string") {
        if (item.imageData2.length > 7_000_000) {
          errors.push({
            field: `baggageItems[${idx}].imageData2`,
            message: "Segunda imagem muito grande (máx. 5MB)",
          });
        }
      }
    });
...
const sanitizeInput = (body) => {
  const phone = cleanPhone(body.passengerPhone);
  const cpfClean = body.passengerCpf
    ? body.passengerCpf.replace(/\D/g, "")
    : null;
  ...
  return {
    ...
    sunNumber: body.sunNumber
      ? sanitize(String(body.sunNumber)).toUpperCase()
      : null,
