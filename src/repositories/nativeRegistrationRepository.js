...
async function tryPrisma(fn, memFallback) {
  ...
  try {
    ...
    if (isDbError) {
      console.warn(
        "[NativeRepo] DB/Prisma indisponível, fallback memória:",
        err.message,
      );
      dbFailed = true;
      return memFallback();
...
function matchFilter(record, filters) {
  ...
  if (
    filters.passengerEmail &&
    !record.passengerEmail
      .toLowerCase()
      .includes(filters.passengerEmail.toLowerCase())
  )
    return false;
  if (
  ...
  if (
    ...
    !(record.cpvNumber || "")
      .toLowerCase()
      .includes(filters.cpvNumber.toLowerCase())
  )
    return false;
  if (
    filters.hasInsurance !== undefined &&
    !!record.hasInsurance !== filters.hasInsurance
  )
    return false;
  if (
    filters.isPhysicalTag !== undefined &&
    !!record.isPhysicalTag !== filters.isPhysicalTag
  )
    return false;
  if (
    filters.dateFrom &&
    ...
    return false;
  if (filters.dateTo && new Date(record.createdAt) > new Date(filters.dateTo))
    return false;
  return true;
}
...
const prismaCreate = async (data) => {
  const { baggageItems, physicalTag, sunNumber, insured, ...registrationData } =
    data;
...
const prismaFindById = async (id) => {
  return prisma.nativeRegistration.findUnique({
    where: { id },
    include: { baggageItems: true },
  });
};
...
const prismaFindMany = async ({
  ...
  if (filters.passengerEmail)
    where.passengerEmail = {
      contains: filters.passengerEmail,
      mode: "insensitive",
    };
  if (filters.passengerName)
    where.passengerName = {
      contains: filters.passengerName,
      mode: "insensitive",
    };
  if (filters.cpvNumber)
    where.cpvNumber = { contains: filters.cpvNumber, mode: "insensitive" };
  if (filters.hasInsurance !== undefined)
    where.hasInsurance = filters.hasInsurance;
  if (filters.isPhysicalTag !== undefined)
    where.isPhysicalTag = filters.isPhysicalTag;
  ...
  const data = records.map((r) => ({
    ...r,
    photoCount: r.baggageItems
      ? r.baggageItems.reduce(
          (n, i) => n + (i.imageData ? 1 : 0) + (i.imageData2 ? 1 : 0),
          0,
        )
      : 0,
  }));
...
const prismaCountByStatus = async (filters = {}) => {
  ...
  const result = await prisma.nativeRegistration.groupBy({
    by: ["status"],
    where,
    _count: { _all: true },
  });
...
const memCreate = async (data) => {
  ...
  const record = {
    ...
    ...rest,
    baggageItems: (baggageItems || []).map((b) => ({
      id: crypto.randomUUID(),
      ...b,
      createdAt: now,
    })),
...
const memFindByCpvNumber = async (cpvNumber) =>
  memStore.find((r) => r.cpvNumber === cpvNumber) || null;
...
module.exports = {
  ...
  findByCpvNumber: (cpv) =>
    tryPrisma(
      () => prismaFindByCpvNumber(cpv),
      () => memFindByCpvNumber(cpv),
    ),
  findRecent: (email, phone, windowMs) =>
    tryPrisma(
      () => prismaFindRecent(email, phone, windowMs),
      () => memFindRecent(email, phone, windowMs),
    ),
  update: (id, data) =>
    tryPrisma(
      () => prismaUpdate(id, data),
      () => memUpdate(id, data),
    ),
  findMany: (opts) =>
    tryPrisma(
