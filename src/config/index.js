...
if (process.env.NODE_ENV !== "test") {
  ...
  if (missing.length) {
    console.error(
      `[config] FATAL: Missing required env var(s): ${missing.join(", ")}`,
    );
    process.exit(1);
  }
}
...
if (process.env.DATABASE_URL && process.env.NODE_ENV !== "test") {
  try {
    ...
    const connLimit =
      dbUrl && !dbUrl.includes("connection_limit")
        ? dbUrl.includes("?")
    ...
    console.warn(
      `[config] Prisma client unavailable, falling back to non-persistent mode: ${error.message}`,
    );
    prisma = null;
  }
}
...
const MINIO_ENDPOINT =
  process.env.MINIO_ENDPOINT || process.env.MINIO_PRIVATE_ENDPOINT || "";
