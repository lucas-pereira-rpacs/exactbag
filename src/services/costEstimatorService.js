...
const ESTIMATES = {
  ...
  whatsapp: {
    ...
    costPerMessageUSD: Number(process.env.WHATSAPP_COST_PER_MESSAGE || getDefaultWhatsAppCost()),
    twilioCost: 0.04,
    metaCost: 0.0085,
    zapiCost: 0.02
  }
...
function getDefaultWhatsAppCost() {
  ...
  switch (provider) {
    case 'meta': return 0.0085;
    case 'zapi': return 0.02;
    case 'twilio':
    default: return 0.04;
  }
}
...
function estimateMonthlyCost({ emailsPerMonth, whatsappMessagesPerMonth }) {
  const emailUnits = Math.max(0, emailsPerMonth - ESTIMATES.email.freeTierPerMonth);
  const emailCost = emailUnits * ESTIMATES.email.costPerEmailUSD;
  const whatsappCost = (whatsappMessagesPerMonth || 0) * ESTIMATES.whatsapp.costPerMessageUSD;
