...
class NotificationService {
  ...
  async sendPurchaseConfirmation(customerData, saleData, options = {}) {
    const { useEmail = true, useWhatsapp = true } = options;

    const notifications = [];

    console.log(
      `[NotificationService] Enviando confirmação de compra para ${customerData.name}`,
    );
    ...
    if (useEmail && emailGateway.sendConfirmationEmail) {
      notifications.push(
        emailGateway
          .sendConfirmationEmail(customerData, {
            customerData,
            saleData,
          })
          .catch((err) => {
            console.error(
              "[NotificationService] Erro ao enviar email de confirmação:",
              err.message,
            );
    ...
    if (useWhatsapp && whatsappGateway.sendConfirmationMessage) {
      notifications.push(
        whatsappGateway
          .sendConfirmationMessage(customerData, {
            customerData,
            saleData,
          })
          .catch((err) => {
            console.error(
              "[NotificationService] Erro ao enviar WhatsApp de confirmação:",
              err,
            );
  ...
  async sendPurchaseNotification(customerData, saleData, registrationLink) {
    console.log(
      `[NotificationService] Enviando notificação de compra para ${customerData.name}`,
    );
    ...
    if (emailGateway.sendPurchaseTemplateEmail) {
      results.push(
        emailGateway
          .sendPurchaseTemplateEmail(customerData, safeLink)
          .catch((err) => {
            console.error(
              "[NotificationService] Erro email compra:",
              err.message,
            );
  ...
  async sendPurchaseConfirmationNotification(passengerData, saleData) {
    ...
    console.log(
      `[NotificationService] Enviando confirmacao de compra para ${passenger.name} <${passenger.email}>`,
    );
    ...
    const details = results.map((result, index) => {
      ...
      console.error(
        `[NotificationService] Erro ${channels[index]} confirmacao de compra:`,
        result.reason,
      );
      return {
        channel: channels[index],
  ...
  async sendPurchaseReminderNotification(
    customerData,
    saleData,
    registrationLink,
  ) {
  ...
  async sendVesperaNotification(customerData, saleData, registrationLink) {
    console.log(
      `[NotificationService] Enviando notificação véspera para ${customerData.name}`,
    );
    ...
    if (emailGateway.sendVesperaTemplateEmail) {
      results.push(
        emailGateway
          .sendVesperaTemplateEmail(customerData, registrationLink)
          .catch((err) => {
            console.error(
              "[NotificationService] Erro email véspera:",
              err.message,
            );
            return { channel: "email", success: false, error: err.message };
          }),
    ...
    if (whatsappGateway.sendTemplate) {
      results.push(
        whatsappGateway
          .sendTemplate(customerData.phone, "exactbag_vespera_voo", "pt_BR", [
            {
              type: "body",
              parameters: [
                { type: "text", text: customerData.name },
  ...
  async sendReminder(customerData, message, channel = "both") {
    console.log(
      `[NotificationService] Enviando lembrete via ${channel} para ${customerData.name}`,
    );
    ...
    if ((channel === "email" || channel === "both") && emailGateway.sendEmail) {
      reminders.push(
        emailGateway
          .sendEmail(customerData.email, "Lembrete ExactBag", message)
          .catch((err) => false),
      );
    }
