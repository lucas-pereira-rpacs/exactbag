...
try {
  CPV_EMAIL_TEMPLATE = fs.readFileSync(
    path.resolve(__dirname, "..", "views", "email_cpv.html"),
    "utf-8",
  );
  console.log("[NativeNotification] Template de e-mail CPV carregado");
} catch (_err) {
  console.warn(
    "[NativeNotification] Template CPV não encontrado, usando fallback.",
  );
}
...
const sendCpvByEmail = async (
  ...
  if (CPV_EMAIL_TEMPLATE) {
    html = CPV_EMAIL_TEMPLATE.replace(/\{\{nome\}\}/g, name)
  ...
  console.log(
    `[NativeNotification] CPV ${cpvNumber} enviado por e-mail para ${email}`,
  );
  return true;
};
...
const sendPhysicalTagRegistrationEmail = async (registration) => {
  ...
  const title = insured
    ? "Sua TAG ExactBag COVER foi Registrada!"
    : "Sua TAG ExactBag foi Registrada!";
  ...
  const content = insured
    ...
    : '<p><strong>ATENÇÃO ⚠️:</strong></p><p>A ExactBag <strong>NÃO</strong> faz rastreamento de bagagens por geolocalização.<br>Nosso serviço consiste em:</p><ul><li><strong>Identificar a sua bagagem</strong> por meio da TAG exclusiva;</li><li><strong>Gerar evidências</strong> para te proteger em caso de problemas;</li><li><strong>Colaborar com a companhia aérea</strong> para agilizar a solução.</li></ul><p>Se tiver <strong>qualquer problema</strong> com sua bagagem, basta nos acionar — nossa equipe está disponível <strong>24 horas</strong> por dia para te ajudar.</p><p><em><strong>Teve problemas com sua bagagem? Conte com a gente!</strong></em></p><p><a href="https://wa.me/' +
      supportPhoneRaw +
      '">WhatsApp: ' +
      supportPhone +
      '</a><br><a href="mailto:contato@exactbag.com.br">E-mail: contato@exactbag.com.br</a></p><p><em>Saiba mais do nosso serviço: <a href="' +
      link +
      '">Como Funciona a TAG ExactBag</a></em></p>';
  const html =
    '<!doctype html><html lang="pt-BR"><body style="margin:0;background:#f5f5ff;font-family:Arial,sans-serif;color:#111;font-size:14px;line-height:1.35"><div style="max-width:600px;margin:16px auto;background:#fff;padding:28px 18px 22px"><div style="text-align:center;margin-bottom:28px"><div style="font-size:58px;color:' +
    accent +
    '">✉✓</div><h1 style="font-size:32px;line-height:1.05;margin:12px 0 0">' +
    title +
    "</h1></div>" +
    content +
    '<p style="margin-top:22px">📩 Logo você receberá uma cópia do seu registro ExactBag por e-mail.</p><div style="text-align:center;margin-top:24px"><a href="' +
    link +
    '" style="display:inline-block;padding:10px 18px;border:1px solid #555;border-radius:5px;color:#222;text-decoration:none">Preencher novamente</a></div></div></body></html>';
  const text =
    title +
    "\n\nAcesse novamente: " +
    link +
  ...
  await emailGateway.sendEmail({
    to: registration.passengerEmail,
    subject: title,
    html,
    text,
  });
  console.log(
    "[NativeNotification] Confirmação de TAG física enviada para",
    registration.passengerEmail,
  );
  return true;
};
...
const sendCpvByWhatsApp = async (registration) => {
  ...
  const name = registration.passengerName;
  const cpvNumber = registration.cpvNumber;
  const registrationType = registration.isPhysicalTag
    ? "physical-tag"
    : "digital";

  if (!phone) {
    console.warn(
      `[NativeNotification] Telefone ausente, WhatsApp não enviado (type: ${registrationType}, CPV: ${cpvNumber})`,
    );
    return false;
  }
  ...
  const templateName = registration.isPhysicalTag
    ? process.env.WHATSAPP_PHYSICAL_TAG_CPV_TEMPLATE ||
  ...
  if (templateName) {
    try {
      ...
      console.log(
        `[NativeNotification] CPV ${cpvNumber} enviado por WhatsApp (type: ${registrationType}, template: ${templateName}) para ${phone}`,
      );
      return true;
    } catch (err) {
      console.warn(
        "[NativeNotification] Template falhou, tentando texto livre:",
        err.message,
      );
    }
  }
  ...
  try {
    ...
    console.log(
      `[NativeNotification] CPV ${cpvNumber} enviado por WhatsApp (type: ${registrationType}, texto) para ${phone}`,
    );
    return true;
  } catch (err) {
...
const sendReturnReminderByEmail = async (registration, registrationLink) => {
  ...
  console.log(
    `[NativeNotification] Lembrete de volta enviado por e-mail para ${registration.passengerEmail}`,
  );
  return true;
};
...
const sendReturnReminderByWhatsApp = async (registration, registrationLink) => {
  ...
  if (!phone) {
    console.warn(
      "[NativeNotification] Telefone ausente, WhatsApp volta não enviado",
    );
    return false;
  }
  ...
  if (templateName) {
    try {
      ...
      console.log(
        `[NativeNotification] Lembrete volta enviado por WhatsApp (template: ${templateName}) para ${phone}`,
      );
      return true;
    } catch (err) {
      console.warn(
        "[NativeNotification] Template volta falhou, tentando texto livre:",
        err.message,
      );
    }
  }
  ...
  try {
    await whatsappGateway._sendMessage(phone, message);
    console.log(
      `[NativeNotification] Lembrete volta enviado por WhatsApp (texto) para ${phone}`,
    );
    return true;
  } catch (err) {
