...
const saleCancellationService = {
  ...
  async cancelSale(saleId, partnerId, reason) {
    ...
    if (sale.status === 'cancelada') {
      throw Object.assign(new Error('Venda já está cancelada'), { statusCode: 409 });
    }

    const cancellableStatuses = ['reserva', 'reserva_paga', 'processing', 'processed'];
    ...
    this._notifyPartner(updatedSale, reason).catch((err) =>
      console.error('[Cancellation] Erro ao notificar parceiro:', err.message)
    ); 
  ...
  async _notifyPartner(sale, reason) {
    ...
    if (partner.webhookUrl) {
      ...
      const success = await partnerCallbackService._postWithTimeout(partner.webhookUrl, payload, 5000);
      if (success) {
        console.log(`[Cancellation] Callback de cancelamento enviado para ${partner.partnerId}`);
        return;
      }
    }
