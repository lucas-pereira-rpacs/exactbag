...
try {
  ...
  console.warn(
    `[swagger] Failed to load OpenAPI from ${openApiPath}: ${error.message}`,
  );
  swaggerDocument = {
    openapi: "3.0.0",
    info: {
      title: "ExactBag Partner API",
      version: "1.0.0",
      description: "OpenAPI temporariamente indisponivel.",
    },
    paths: {},
  };
}
...
if (process.env.NATIVE_REGISTRATION_ENABLED !== "false") {
  ...
  app.get("/registrodebagagem", publicLimiter, async (req, res) => {
    ...
    if (saleId) {
      const [sale] =
        await prisma.$queryRaw`SELECT * FROM "Sale" WHERE "saleId" = ${saleId} LIMIT 1`;
      const isExpired =
        sale?.expirationDate && new Date(sale.expirationDate) <= new Date();
  ...
  app.use(
    "/native/assets",
    publicLimiter,
    express.static(path.join(nativePath, "assets"), {
      maxAge: "7d",
      etag: true,
      lastModified: true,
      fallthrough: false,
