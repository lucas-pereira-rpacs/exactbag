...
async function agendaJobCleanupHandler(_job) {
  const configuredHours = Number.parseInt(process.env.AGENDA_JOB_RETENTION_HOURS, 10);
  const retentionHours = Number.isInteger(configuredHours) && configuredHours > 0
    ? configuredHours
    : 30 * 24;
  ...
  if (result.rowCount > 0) {
    console.log(`[Agenda][cleanup] Removed ${result.rowCount} business jobs older than ${retentionHours}h`);
  }
}
