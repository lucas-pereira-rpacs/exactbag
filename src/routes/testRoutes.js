...
router.post("/trigger/purchase-template", async (req, res) => {
  try {
    ...
    if (!name || !email || !phone) {
      return res
        .status(400)
        .json({ error: "name, email e phone são obrigatórios" });
    }
    ...
    if (!registrationLink) {
      return res
        ...
        .json({
          error:
            "Nenhuma venda encontrada para este cliente. Informe registrationLink manualmente.",
        });
    }
...
router.post("/return-reminder", async (req, res) => {
  try {
    ...
    res.json({
      status: "ok",
      message: `Lembrete de volta enviado para ${targetEmail}`,
    });
  } catch (error) {
