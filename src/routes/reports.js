...
const { buildPublicFormUrl, buildPrettyPublicFormUrl } = require('../services/publicFormLinkService');
...
router.get('/sales/:saleId', async (req, res) => {
  try {
    ...
    res.status(200).json({
      status: 'success',
      sale,
      shortFormLink: sale.formLink ? buildPublicFormUrl(req, sale.saleId) : null,
      prettyFormLink: sale.formLink ? buildPrettyPublicFormUrl(req, sale.slug || sale.customerName) : null
    });
  } catch (error) {
...
router.get('/preview', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="relatorio-preview.csv"');
    res.send(csv);
  } catch (error) {
