...
} = require("../controllers/physicalTagSaleController");
const {
  validateSun,
...
router.post("/physical-tag/validate-sun", async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        valid: false,
        error: "Não foi possível validar o SUN. Tente novamente.",
      });
  }
});

router.get(
  "/physical-tag-factory",
  dashboardAuthMiddleware,
  ...
  async (_req, res) => {
    try {
      ...
      return res
        ...
        .json({
          success: false,
          error: "Erro ao carregar configuração de SUN.",
        });
    }
  },
);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "[PhysicalTagFactory] Erro ao atualizar configuração:",
        error,
      );
      return res
        .status(500)
        .json({
          success: false,
          error: "Erro ao atualizar configuração de SUN.",
        });
    }
  },
);
  ...
  async (_req, res) => {
    try {
      ...
      res.set({
        ...
        "Content-Disposition":
          'attachment; filename="exactbag-registro-tag-fisica.svg"',
      });
      ...
      return res
        .status(500)
        .json({ success: false, error: "Erro ao gerar QR Code." });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (prisma) {
        const [sales, tagRows] = await Promise.all([
          ...
          SELECT id, "orderNumber", product, "customerName", "customerEmail",
                 "customerPhone", "outboundDate", "receiptSentAt", quantity, "hasInsurance", "partnerId", "operatorEmail", "createdAt"
      ...
      return res
        ...
        .json({
          success: false,
          error: "Erro ao carregar histórico de vendas.",
        });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (!sale)
        return res
          .status(404)
          .json({ success: false, error: "Venda não encontrada." });

      const sent = [];
      const sentAt = (label, at, source) => sent.push({ label, at, source });
      const isJustTravel =
        String(sale.partnerId || "")
          .replace(/[\s_-]/g, "")
          .toLowerCase() === "justtravel";
      const outboundTime = sale.outboundDate
        ? new Date(sale.outboundDate).getTime()
        : NaN;
      ...
      const wasDeferredConfirmation =
        ...
        outboundTime - createdTime > 48 * 60 * 60 * 1000;
      const welcomeAndVesperaAreSameEmail =
        sale.welcomeSentAt &&
        sale.vesperaSentAt &&
        Math.abs(
          new Date(sale.welcomeSentAt).getTime() -
            new Date(sale.vesperaSentAt).getTime(),
        ) < 5000;

      if (sale.welcomeSentAt) {
        sentAt(
          wasDeferredConfirmation
            ? "Confirmação de compra"
            : "E-mail com link de registro",
          sale.welcomeSentAt,
          "sale",
        );
      }
      if (sale.vesperaSentAt && !welcomeAndVesperaAreSameEmail) {
        sentAt(
          "E-mail com link de registro (scheduler de 48h)",
          sale.vesperaSentAt,
          "scheduler",
        );
      }
      ...
      if (sale.status !== "processed") {
        scheduled.push({
          ...
          status: "blocked",
          reason: "A venda ainda não está processada.",
        });
      } else if (!sale.outboundDate || !Number.isFinite(outboundTime)) {
        scheduled.push({
          label: "E-mail com link de registro (scheduler de 48h)",
          status: "blocked",
          reason: "Data da viagem não informada.",
        });
      } else if (sale.vesperaSentAt) {
        scheduled.push({
          label: "E-mail com link de registro (scheduler de 48h)",
          status: "sent",
          at: sale.vesperaSentAt,
        });
      } else {
        const scheduledAt = new Date(outboundTime - 48 * 60 * 60 * 1000);
        const now = Date.now();
        const inSchedulerWindow =
          outboundTime - now >= 47 * 60 * 60 * 1000 &&
          outboundTime - now <= 49 * 60 * 60 * 1000;
        scheduled.push({
          label: "E-mail com link de registro (scheduler de 48h)",
          status: inSchedulerWindow
            ? "due"
            : scheduledAt > new Date(now)
              ? "scheduled"
              : "missed-window",
          at: scheduledAt,
          reason: inSchedulerWindow
            ? "Será enviado no próximo ciclo horário do scheduler."
            : undefined,
        });
      }
      ...
      return res
        ...
        .json({
          success: false,
          error: "Erro ao carregar notificações da venda.",
        });
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    ...
    `;
      ...
      return res
        .status(500)
        .json({ success: false, error: "Erro ao carregar integrações NOW." });
    }
  },
);
...
router.get(
  "/registro/:id/image/:itemId/:field",
  controller.streamRegistrationImage,
);
router.get(
  "/registro/:id",
  dashboardAuthMiddleware,
