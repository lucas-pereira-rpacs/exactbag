...
router.post('/data-access', async (req, res) => {
  try {
    ...
    await privacyService.logConsent(email, {
    ...
    res.status(200).json({
      ...
      data,
      notes: 'Estes dados serão enviados para seu email por segurança. Verifique sua caixa de entrada.'
    });
  } catch (error) {
...
router.delete('/data-deletion', async (req, res) => {
  try {
    ...
    console.warn(`[Privacy] Solicitação de deleção de dados de: ${email}, Motivo: ${reason}`);
...
router.post('/data-export', async (req, res) => {
  try {
    ...
    console.log(`[Privacy] Solicita de export de dados de: ${email}, Formato: ${format}`);
