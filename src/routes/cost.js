...
router.get('/estimate', (req, res) => {
  ...
  if (emailsPerMonth < 0 || whatsappMessagesPerMonth < 0) {
    return res.status(400).json({ success: false, error: 'Parâmetros devem ser não negativos' });
  }

  const details = costEstimator.estimateMonthlyCost({ emailsPerMonth, whatsappMessagesPerMonth });
