...
const generateCpvHtml = async (registration) => {
  ...
  let qrDataUri = '';
  try {
    qrDataUri = await _getCachedQr(cpvUrl, { width: 160, margin: 1, color: { dark: '#1e3a5f', light: '#ffffff' } });
  } catch (err) {
...
function _resetBrowserIdleTimer() {
  ...
  _browserIdleTimer = setTimeout(async () => {
    if (_browser) {
      console.log('[CpvPdfService] Chromium ocioso por 60s — fechando para liberar RAM');
      await closeBrowser();
    }
...
const generateCpvPdf = async (registration) => {
  ...
  const html = await generateCpvHtml({ ...registration, cpvNumber });

  const browser = await getBrowser();
  if (!browser) {
    console.warn('[CpvPdfService] Nenhum Chrome/Chromium encontrado, retornando HTML fallback');
